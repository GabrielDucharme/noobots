/**
 * Simplified server.js for Raspberry Pi
 * This is a minimal version of the server that focuses on camera and WebSocket functionality
 */
...
app.get('/camera/test-image', (req, res) => {
    ...
    // Try to respond with a simple colored rectangle if canvas isn't available
    res.writeHead(200, {
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
        ...
        <circle cx="${centerX}" cy="${centerY}" r="${radius}" fill="red" />
        <text x="${centerX}" y="${centerY}" font-family="Arial" font-size="16" fill="white" text-anchor="middle">Test Image</text>
        <text x="${centerX}" y="${centerY + 24}" font-family="Arial" font-size="12" fill="white" text-anchor="middle">${timestamp}</text>
    </svg>
    `;
